...
const Transactions = () => {
  ...
  const [year, setYear] = useState<number>(currentData.getFullYear());
  const [month, setMonth] = useState<number>(currentData.getMonth() + 1);
  ...
  const handleSearchChange = (event: ChangeEvent<HTMLInputElement>): void => {
    ...
    setFilteredTransactions(
      transactions.filter((transactions) =>
        transactions.description.toUpperCase().includes(event.target.value.toUpperCase()),
      ),
    );
  };
                  ...
                  <tr key={transactions.id} className="hover:bg-gray-800 ">
                    ...
                    <td className="px-3 py-4 whitespace-nowrap">
                      <div className="flex items-center">
                        <div
                          ...
                          style={{ backgroundColor: transactions.category.color }}
                        ></div>
                        <span className="text-sm text-gray-400">{transactions.category.name}</span>
                      </div>
                    </td>
                    ...
                    <td className="px-3 py-4 whitespace-nowrap cursor-pointer">
                      <button
                        ...
                        disabled={deletingId === transactions.id}
                      >
                        {deletingId === transactions.id ? (
